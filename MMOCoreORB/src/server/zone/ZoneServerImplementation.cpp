...
void ZoneServerImplementation::startManagers() {
	...
	//Infinity:  Delay city update until after server start
	int updateCityRegionsDelay = ConfigManager::instance()->getInt("Core3.CityManager.UpdateCityRegionsDelay", 60 * 1000);
	...
	}, "UpdateCityRegionsLambda", updateCityRegionsDelay);

	info("scheduling updateCityRegions task to run in " + String::valueOf(updateCityRegionsDelay / 1000.0f) + " seconds.");
}
