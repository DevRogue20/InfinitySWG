...
void ShuttleZoneComponent::notifyInsertToZone(SceneObject* sceneObject, Zone* zone) const {
	...
	info(true) << "ScheduleShuttleTask for " << zone->getZoneName() << " scheduled due to server loading. Millitime since server start: " << startDiff << ", bootDelay = " << bootDelay << ", delay: " << delay << " miliseconds.";
#endif
