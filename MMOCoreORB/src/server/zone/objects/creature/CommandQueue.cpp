...
void CommandQueue::enqueueCommand(unsigned int actionCRC, unsigned int actionCount, uint64 targetID, const UnicodeString& arguments, int priority, int compareCounter) {
...
#ifdef DEBUG_QUEUE
	if (creature->isPet()) {
		creature->info(true) << "Pet Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ",, Queue Size = " << size << ", isCombat = " << queueCommand->isCombatCommand();
		...
		for (int i = 0; i < queueVector.size(); ++i) {
			...
			creature->info(true) << "Command # " << i+1 << " = "  << queueCommand->getQueueCommandName();
		}
	}
#endif
	...
	//Infinity:  We need to cap pet command queue at 5 commands like AI agent normal priority
	if (priority == QueueCommand::NOCOMBATQUEUE && creature->isPet() && size >= 7) {
#ifdef DEBUG_QUEUE
...
#ifdef DEBUG_QUEUE
	creature->info(true) << "Enqueuing Command " << queueCommand->getQueueCommandName() << ": with priority = " << priority << ", compareCount = " << compareCounter <<
			 ", Queue Size = " << queueVector.size() << ", isCombatCommand = " << queueCommand->isCombatCommand();
#endif
