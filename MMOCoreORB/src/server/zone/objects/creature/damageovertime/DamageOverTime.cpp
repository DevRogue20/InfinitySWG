...
uint32 DamageOverTime::doBleedingTick(CreatureObject* victim, CreatureObject* attacker) {
	...
	Core::getTaskManager()->executeTask([victimRef, attackerRef, attribute, damage] () {
        ...
        if (victimRef == nullptr || attackerRef == nullptr) //Infinity:  Stability check
            return;
...
uint32 DamageOverTime::doForceChokeTick(CreatureObject* victim, CreatureObject* attacker) {
	...
	Core::getTaskManager()->executeTask([victimRef, attackerRef, attribute, strength] () {
        ...
        if (armor != nullptr && !armor->isVulnerable(SharedWeaponObjectTemplate::LIGHTSABER)) {
            ...
            if (attackerRef->isPlayerCreature())	                                                    //Force Choke now has inherent AP2 for players only
                armorPiercing = 2;

            if (attackerRef->isPlayerCreature() && victimRef->hasBuff(BuffCRC::JEDI_FORCEBREACH))       //Increase DOT damage by one AP level when breached
                armorPiercing++;
            ...
            if (armorReduction > 0) {
                ...
                combatManager->sendMitigationCombatSpam(victimRef, armor, (int)dmgAbsorbed, 0x6);
            }
	    }
