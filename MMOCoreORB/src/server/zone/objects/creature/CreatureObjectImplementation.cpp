...
bool CreatureObjectImplementation::isAttackableBy(CreatureObject* creature, bool bypassDeadCheck) {
	...
	if (isPlayerCreature()) {
		...
		if (creature->isPlayerCreature() && creature->isRidingMount()) {
			...
			if (parentCreo != nullptr && parentCreo->isSwimming()) {  // Owner can't attack if riding mount and mount is swimming
				return false;
			}	
		}
		...
		if (creature->isAiAgent()) {
			...
			if (creature->isPet() && (agentCreo != nullptr && !agentCreo->isMindTricked())) {
				...
				if (isPlayerCreature() || isPet()) {  //PVP Pet checks
					...
					if (parentCreo != nullptr && parentCreo->isSwimming())   // Pet's can't attack if owner is mounted and mount is swimming
						return false;
				}
