...
void AiAgentImplementation::respawn(Zone* zone, int level) {
    ...
    unsigned int originalCreatureMask = getCreatureBitmask();   //Infinity:  save original creatureBitmask as it is lost when template reloaded
	reloadTemplate();                                           //Reload all of the agents info
    setCreatureBitmask(originalCreatureMask);                   //Infinity:  Restore original creature bitmask which may have been modified

	// Check to see if the agent is a creature and rolls to spawn as a baby (lairs and dynamic spawns only)
	ManagedReference<SceneObject*> home = homeObject.get();
   
	if (npcTemplate != nullptr && isCreature()) {  //Infinity:  Want to have babies on static spawns also, so moving home nullptr check
		int chance = 1000;  //Infinity:  2000 to 1000 for better chance at destroy mission lairs
		int babiesSpawned = 0;
		...
		auto creatureManager = zone->getCreatureManager();
 		...
 		//Infinity:  Check for ADULTONLY/BABYONLY
		if ((creatureManager != nullptr && (creatureManager->checkSpawnAsBaby(npcTemplate->getTame(), babiesSpawned, chance) && 
			!(getCreatureBitmask() & CreatureFlag::ADULTONLY))) || (getCreatureBitmask() & CreatureFlag::BABYONLY)) {
...
//#define DEBUG_CAMOUFLAGE
	...
	//Infinity:  This is all reworked.   Camo should have 5% better chance than maskscent.

		if (concealMod > 0) {
			//mod += effectiveTarget->getSkillMod("camouflage") / 2;  //Infinity: Not going to give ranger additional bonus if camo applied by another ranger
			mod /= 1.40f; 
		}
	} else {
	...
	/* Infinity:  Makes no sense to use group level here
	...
	*/
	...
	float roll = mod - creatureLevel / 100;  //Infinity
