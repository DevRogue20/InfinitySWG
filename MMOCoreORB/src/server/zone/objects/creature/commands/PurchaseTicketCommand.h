...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		for (int i = 0; i < closeObjects.size(); i++) {
			...
			// Infinity:  Up to 30 meters from travel terminal
		...
		if (departureZone == nullptr) {
			creature->sendSystemMessage("Zone (departurePlanet) does not exist.");
			return GENERALERROR;
		}

		if (arrivalZone == nullptr) {
			creature->sendSystemMessage("Zone (arrivalPlanet) does not exist.");
			return GENERALERROR;
		}
