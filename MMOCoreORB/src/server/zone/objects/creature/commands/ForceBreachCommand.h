...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (res == SUCCESS) {
			...
			if (creatureTarget->isPlayerCreature()) {	//Check for how many enemies players are in 64 meter range of the target
				...
				if (zone != nullptr) {
					...
					for (int i = 0; i < closeObjects->size(); ++i) {
						...
						if (player == creature)  //Don't count player using Force Breach
							continue;

						Locker locker(player, creature);   //Not sure we need to crosslock, but let's try it just in case

						if (player->isInvisible() || !player->isAttackableBy(creatureTarget))
							continue;
			...
			if (forceArmor > 0)
				buff->setSkillModifier("force_armor",forceArmor * -0.35f * groupModifier); 		//now breaches 35% of force armor
			if (forceShield > 0)
				buff->setSkillModifier("force_shield",forceShield * -0.50f * groupModifier); 	//now breaches 50% of force shield
