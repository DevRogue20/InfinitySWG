...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (object->isPlayerCreature() || (object->isCreatureObject() && !object->isVendor())) {
			creature->sendSystemMessage("Players and non-vendor creature objects cannot be rotated with this command.");
			return GENERALERROR;
		}
		...
		if (ghost->getAdminLevel() != 15)  { //// Restrictions for non-admin characters
			...
			if (data != nullptr) {
				...
				if (!isDeco && !object->isMissionTerminal() && !object->getObjectTemplate()->getFullTemplateString().contains("trainer")) {
					creature->sendSystemMessage("You can only rotate objects that are city decorations, mission terminals, or trainers."); 
					return GENERALERROR;
				}
			} 
			else {
