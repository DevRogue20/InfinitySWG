...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (container == "equipment") {
			...
			msg << targetObj->getFirstName() << " is a rank " << playerRank << " " << councilString << " Jedi with " << String::withCommas(targetGhost->getExperience("force_rank_xp")) << " FRS XP.";
			...
			return sendActivePets(creature, targetObj);
	...
	int sendSkillsInfo(CreatureObject* creature, CreatureObject* target, bool eMail = false) const {
		...
		const SkillList* skillList = target->getSkillList();
        StringBuffer body;
	...
	int sendActivePets(CreatureObject* creature, CreatureObject* target) const {
