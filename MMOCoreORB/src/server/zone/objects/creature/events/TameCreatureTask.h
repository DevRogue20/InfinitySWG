...
public:
	TameCreatureTask(Creature* cre, CreatureObject* playo, unsigned int pvpMask, unsigned int creatureMask, bool forced, bool adults) : Task() {  //Infinity:  Added creatureMask
	...
	void resetStatus() {
		...
		if (creature->isAiAgent()) {
			...
			agent->setAITemplate();
			agent->setCreatureBitmask(originalCreatureMask);
		}
	}
};
