...
void WeaponObjectImplementation::fillAttributeList(AttributeListMessage* alm, CreatureObject* object) {
	...
	if (getForceCost() > 0) {
		alm->insertAttribute("forcecost", String::format("%.1f", getForceCost()));  //Infinity:  Show one decimal on force cost 
	}
	...
	if (isJediWeapon() && getMaxDamage() > 750 && object != nullptr && object->checkCooldownRecovery("Lightsaber:" + String::valueOf(getObjectID()))) {
