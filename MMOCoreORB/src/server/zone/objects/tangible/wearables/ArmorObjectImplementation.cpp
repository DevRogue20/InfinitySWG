...
float ArmorObjectImplementation::getTypeValue(int type, float value) const {
    ...
    if (type == SharedWeaponObjectTemplate::LIGHTSABER && !const_cast<ArmorObjectImplementation*>(this)->isPsgArmorObject() && newValue > 39.0f) {  //Infinity:  Hardcap for Mando LS Resist until better resources avaiable
