...
int FishingManagerImplementation::vegetation(SceneObject* marker) {
	...
	int value = (int)((fabs(marker->getPositionX()) + fabs(marker->getPositionY())) / 2.0f / 100.0f + season) % 6; //Shift every season, by location
...
int FishingManagerImplementation::density(SceneObject* marker) {
	...
	Time currentTime;
	uint64 shiftTime = currentTime.getTime() / 60.0f / 60.0f / 21.25f;  //Shift every 21.25 hours, based on time

	int value = (int)((fabs(marker->getPositionX()) + fabs(marker->getPositionY())) / 2.0f / 100.0f + (shiftTime / 1000.0f)) % 6;
...
SceneObject* FishingManagerImplementation::updateMarker(CreatureObject* player, SceneObject* marker, uint32 boxID, bool reelGame) {
	...
	if (reelGame) {
		...
		switch (event) {
			...
			case 3:
				// Fight Hard
				newMarkerLoc.setX(markerLoc.getX() + System::random(20)/10.0f);
				newMarkerLoc.setY(markerLoc.getY() + System::random(20)/10.0f);
			...
			case 5: {
				...
				newMarkerLoc.setX(markerLoc.getX() + sin(angle) * System::random(30)/10.0f);
				newMarkerLoc.setY(markerLoc.getY() + cos(angle) * System::random(30)/10.0f);
...
FishingEvent* FishingManagerImplementation::createFishingEvent(CreatureObject* player, int state) {
	...
	timer = (state >= CATCH) ? timer + 3500 : timer + 5500;
