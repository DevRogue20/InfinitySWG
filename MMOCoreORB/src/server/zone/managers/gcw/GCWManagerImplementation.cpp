...
void GCWManagerImplementation::doBaseDestruction(BuildingObject* building) {
	...
	if (baseType == PLAYERFACTIONBASE && pveBase && zone != nullptr) {
		...
		for (int i = 0; i < closeObjects->size(); ++i) {
			...
			if (object == nullptr || !object->isCreatureObject() || object->isPlayerCreature() || object->isPet())
				continue;

			ManagedReference<CreatureObject*> creature = object->asCreatureObject();

			if (creature == nullptr || creature->isInvisible() || !creature->isAiAgent())
				continue;
