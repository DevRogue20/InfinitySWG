...
void CombatManager::finalCombatSpam(TangibleObject* attacker, WeaponObject* weapon, SortedVector<DefenderHitList*> targetDefenders, const CreatureAttackData& data) const {
	...
	for (int i = 0; i < targetDefenders.size(); ++i) {
		...
		if (defender != nullptr) {
			...
			int copMitigation = hitList->getCopMitigation();
			...
			if (defender->isCreatureObject()) {
				...
				if (defenderCreo != nullptr) {
					...
					if (copMitigation > 0) {
						sendMitigationCombatSpam(defenderCreo, nullptr, copMitigation, CLOAKOFPAIN);
					}
...
float CombatManager::calculateDamage(CreatureObject* attacker, WeaponObject* weapon, CreatureObject* defender, const CreatureAttackData& data) const {
   ...
   if (commandName.beginsWith("flame"))								//Infinity: 25% damage reduction on flamethrower attack in PVP
	...
	if (damage < saveDamage) {
		debugCalculateDamage << " Damage after flamethrower/heavyweapon/special heavy weaapon reduction: " << damage << "\n";
	}
#endif // DEBUG_CALCULATEDAMAGE
	...
	if (data.isForceAttack()) {
		int forceDefense = Math::min(defender->getSkillMod("force_defense"), 125);  // Infinity:  125 Cap
		float forceDefenseMitigation = .45f * (forceDefense/125.0f) * defender->getFrsMod("manipulation");
		if (forceDefense > 0) {
			float adjustedForceDefense = (forceDefense/125.0f) * defender->getFrsMod("manipulation");
			damage *= 1.f - (.45 * adjustedForceDefense); // Infinity: force defense now protects vs force attacks directly and scales with manipulation
#ifdef DEBUG_CALCULATEDAMAGE
			debugCalculateDamage << " Damage after Force Defense reduction: " << damage << ".  FRS-adjusted Force defense: " << adjustedForceDefense <<  ".\n";
#endif // DEBUG_CALCULATEDAMAGE
		}
	}
   ...
   if (!isDelayed && defenderGhost != nullptr && defender->hasBuff(BuffCRC::JEDI_FORTIFY) && !attacker->isIntimidated() &&
	...
	if (data.isForceAttack()) {
		auto wielded = attacker->getWeapon();
...
#ifdef DEBUG_CALCULATEDAMAGE
				debugCalculateDamage << " Damage after force lightning and throw reduction (50% PVP, 80% from NPC): " << damage << "\n";
#endif // DEBUG_CALCULATEDAMAGE	
			}
		}
	...
	if (!data.isForceAttack() && defender->isPlayerCreature() && attacker->isPlayerCreature() && weapon->isJediWeapon() & defenderMeleeWeapon & !defenderJediWeapon) { 
...
int CombatManager::calculateDamageRange(TangibleObject* attacker, CreatureObject* defender, WeaponObject* weapon) const {
	...
	if (defenderGhost != nullptr) {
		...
		if (defender->isPlayerCreature() && !(attacker->isPlayerCreature() || attacker->isPet())) {
...
void CombatManager::applyDots(CreatureObject* attacker, CreatureObject* defender, const CreatureAttackData& data, int appliedDamage, int unmitDamage, int poolsToDamage) const {
	...
	for (int i = 0; i < dotEffects->size(); i++) {
		...
		if (data.getCommandName().beginsWith("forcelightning") && !attacker->isPlayerCreature()) // Infinity:  No lightning fire DOTs for npcs
			continue;
		...
		error("DOT Type = " + dotName + ", Pool = " + poolName + 
				", Primary DOT = " + String::valueOf(effect.isDotDamageofHit() ? damageToApply * effect.getPrimaryPercent() / 100.0f : dotStrength) +
				", Secondary DOT = " + String::valueOf(effect.isDotDamageofHit() ? damageToApply * effect.getSecondaryPercent() / 100.0f : dotStrength) +
				"potency = " +  String::valueOf(potency) + ", resist = " + String::valueOf(resist));

		//DOT Logging Ends Here */

		defender->addDotState(attacker, dotType, data.getCommand()->getNameCRC(), effect.isDotDamageofHit() ? damageToApply * effect.getPrimaryPercent() / 100.0f : dotStrength, pool, effect.getDotDuration(), potency, resist,
							  effect.isDotDamageofHit() ? damageToApply * effect.getSecondaryPercent() / 100.0f : dotStrength);
	}
}
...
int CombatManager::getAttackerAccuracyModifier(TangibleObject* attacker, CreatureObject* defender, WeaponObject* weapon) const {
	...
	if (weapon->getAttackType() == SharedWeaponObjectTemplate::MELEEATTACK) {
		melee_accuracy = creoAttacker->getSkillMod("melee_accuracy");
		attackerAccuracy += melee_accuracy;
	}
	...
	if (weapon != nullptr && weapon->getAttackType() == SharedWeaponObjectTemplate::MELEEATTACK && weapon->isJediWeapon() && creoAttacker->isPlayerCreature()) {
		...
		// Infinity: allow for buff stims taking enhanced accuracy over 12 from skill tree
		if (melee_accuracy > 12)
			attackerAccuracy += melee_accuracy - 12;
	}
...
int CombatManager::getHitChance(TangibleObject* attacker, CreatureObject* creoDefender, WeaponObject* weapon, const CreatureAttackData& data, int damage, int bonusAccuracy) const {
	...
	if (data.isForceAttack()) {
		if (creoAttacker != nullptr) {
			// Infinity:  Force powers almost almost always hits, will use a base accuracy of 100, then add bonus accuracy from command lua
			//accuracySkill = creoAttacker->getSkillMod(data.getCommand()->getAccuracySkillMod());
			accuracySkill = 100 * creoAttacker->getFrsMod("manipulation",1.67);  // Infinity:  Adjust powers accuracy based on FRS manipulation mod up to 10%
	...
	if (hitResult == HitStatus::HIT && !data.isForceAttack() && !data.isStateOnlyAttack()) {
		...
		if (defendResult == HitStatus::RICOCHET) {
			...
			if (attackType == SharedWeaponObjectTemplate::RANGEDATTACK && !attacker->isTurret() && !weapon->isThrownWeapon() && defendWeapon != nullptr && defendWeapon->isJediWeapon() && !creoDefender->isWearingArmor()) {
                ...
                float saberFRSMod = creoDefender->getFrsMod("manipulation",0.33);  // Max 80 manipulation yields 1.417 value maximum
...
int CombatManager::getArmorReduction(TangibleObject* attacker, WeaponObject* weapon, CreatureObject* defender, DefenderHitList* hitList, float damage, int hitLocation, const CreatureAttackData& data) const {
	...
	if (data.getCommandName().beginsWith("forcelightning") && !defender->isPlayerCreature() && !defender->isPet())    //Infinity:  Lightning is LS Damage in PVE (not player/pet PVP)
	...
	if (armor != nullptr && !armor->isVulnerable(damageType)) {
		...
		if (data.getCommandName().beginsWith("forcelightning") && attacker->isPlayerCreature()) //Infinity:  Force Lightning now has inherent AP2 for players only
			armorPiercing = 2;	

		if (data.getCommandName() == "forcechoke" && attacker->isPlayerCreature())	//Infinity:  Force Choke now has inherent AP2 for players only
			armorPiercing = 2;

		if (data.getCommandName().beginsWith("flame") && attacker->isPlayerCreature()) // Infinity:  AP2 to compensate for 25% damage nerf, equals less damage for unarmored targets (Jedi) and roughly
	...
	for (int i = 0; i < stateEffects->size(); i++) {
			...
			if (creature->isPlayerCreature() && targetCreature->isPlayerCreature()) {
				if (creature->hasSkill("force_title_jedi_rank_03") || (creatureWeapon != nullptr && creatureWeapon->isJediWeapon())) {
					if (targetCreature->hasSkill("force_title_jedi_rank_03") || (targetWeapon != nullptr && targetWeapon->isJediWeapon())) {
						if (data.getCommandName().beginsWith("saber") || data.isForceAttack()) { // Infinity:  Reduce state accuracy in Jedi vs.Jedi PVP by 15 for skills that were increased
							if (accuracyMod >= 80) {
#ifdef DEBUG_STATES
		...
		if (effectType == CommandEffect::INTIMIDATE && (commandName == "fastblast" || commandName == "firelightningcone2" || commandName == "firelightningsingle2")) { 
			if (!creature->isPlayerCreature()) {
				...
				auto creatureGhost = creature->getPlayerObject();
		...
		if (!failed && (effectType == CommandEffect::KNOCKDOWN || effectType == CommandEffect::POSTUREDOWN || effectType == CommandEffect::POSTUREUP)) {
			...
			// Infinity:  Removed intimidate check on combat equilibrium
