...
void FrsManagerImplementation::handleSkillRevoked(CreatureObject* player, const String& skillName) {
	...
	if (skillRank < 0) {
		...
		StringBuffer msg;
		msg << "Skill " << skillName <<  " revoked for player " << player->getFirstName() << ".  Calling setPlayerRank";
		error(msg);
...
void FrsManagerImplementation::promotePlayer(CreatureObject* player) {
	...
	// Infinity
	StringBuffer msg;
	msg << "FRS: Player: " << player->getFirstName() << " has enough xp to rank, promoting to rank: [" <<  newRank << "].  Calling setPlayerRank";
	error(msg);
...
void FrsManagerImplementation::demotePlayer(CreatureObject* player) {
	...
	// Infinity
	StringBuffer msg;
	msg << "FRS: Player: " << player->getFirstName() << " does not have the FRS XP to maintain rank, demoted to rank: [" <<  newRank << "].  Calling setPlayerRank";
	error(msg);
...
void FrsManagerImplementation::deductMaintenanceXp(CreatureObject* player) {
	...
	if (ConfigManager::instance()->getBool("Core3.FrsManager.ImmediateMaintXpDeduction", false)) {
		...
		String msg = "You have lost " + String::withCommas(maintXp) + " Force Rank experience. All members of Rank 1 or higher must pay experience each week to remain in their current positions.";

		if (curDebt > 0) {
			maintXp += curDebt;
			msg = "You have lost " + String::withCommas(maintXp) + " Force Rank experience. This includes " + String::withCommas(curDebt) + " previously banked experience debt. All members of Rank 1 or higher must pay experience each week to remain in their current positions.";
			managerData->removeExperienceDebt(playerID);
		}
		chatManager->sendMail("Enclave Records", "@force_rank:xp_maintenance_sub", msg, player->getFirstName());
