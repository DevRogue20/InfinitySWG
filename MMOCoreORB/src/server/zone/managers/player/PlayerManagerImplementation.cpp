...
void PlayerManagerImplementation::killPlayer(TangibleObject* attacker, CreatureObject* player, int typeofdeath, bool isCombatAction) {
	...
	auto ghost = player->getPlayerObject();  //Infinity:  Let's get ghost up front it is used everywhere
		...
		if (ConfigManager::instance()->isPvpBroadcastChannelEnabled() && ghost != nullptr) {
			...
			if (zoneServer != nullptr) {
				...
				if (chatManager != nullptr) {
					...
					if (attackerCreature->getLastName() != "")
						attackerName += " " + attackerCreature->getLastName();
					...
					if (attackerGhost != nullptr) {
						jediState = attackerGhost->getJediState();
	...
	calculateFRS(attackerCreature, player);
		...
		if (attacker->isPet()) {
			...
			for (int j = 0; j < entry->size(); ++j) {
				...
				if (xpType == "jedi_general") {
					...
					awardExperience(attackerCreo, xpType, jediXp, false, 1.0f, false);  //Award individual experience (Jedi).  No XP modifiers apply.
				}
				else {
			...
			//Vector3 pos(attacker->getWorldPosition());  //Not used
...
SortedVector<String> PlayerManagerImplementation::getTeachableSkills(CreatureObject* teacher, CreatureObject* student) {
	...
	for (int i = 0; i < skillList->size(); ++i) {
		...
		if (!(skillName.contains("force_sensitive") || skillName.contains("force_rank") || skillName.contains("force_title") || skillName.contains("admin_") ||
				 skillName.contains("mandalorian_") || skillName.contains("big_game_") || skillName.contains("combat_demolition") || skillName.contains("stadium"))
...
void PlayerManagerImplementation::sendAdminFRSList(CreatureObject* player) {
	...
	for (int i = 0; i < frsJediList.size(); i++) {
		...
		auto sceneObject = zoneServer->getObject(playerID);
		if (sceneObject == nullptr)
			continue;
		
		auto playerCreo = sceneObject->asCreatureObject();
		if (playerCreo == nullptr)
			continue;

		auto ghost = playerCreo->getPlayerObject();
		if (ghost == nullptr)
			continue;
...
void PlayerManagerImplementation::calculateFRS(CreatureObject* killer, CreatureObject* victim) {
	...
	auto killerGhost = killer->getPlayerObject();
	auto victimGhost = victim->getPlayerObject();

	if (killerGhost == nullptr || victimGhost == nullptr) {
		return;
	}

	if (killer == victim) {    // No PVP / FRS progress if you kill yourself
		return;
	}
	...
	bool bhVictim = victim->hasSkill("combat_bountyhunter_investigation_03");
	...
	if (!victim->isRebel() && !victim->isImperial() && !bhVictim) {
		if (verboseLogging) {
	...
	float killerGroupPower = 1.0;
	int killerGroupId = 0;
	int killerGroupSize = 1;

	killerGroupPower = getPowerRating(killer);
	...
	if (verboseLogging) {
		error("threatMap size = " + String::valueOf(victimThreatMap.size()));
	}
		...
		if (highestFrsRank.contains(attackerAccountID)) {
			...
			if (storedMember != nullptr) {
				...
				if (attackerRank > storedRank) {   //Drop old creo that is of lesser rank
					//error("Dropping account ID: " + String::valueOf(accountID) + ", Player: " + storedMember->getFirstName() + ", Rank: " + String::valueOf(storedRank));
					highestFrsRank.drop(attackerAccountID);
				}
			}
		}							

		//error("Adding account ID: " + String::valueOf(accountID) + ", Player: " + member->getFirstName() + ", Rank: " + String::valueOf(getRank(member)));
	...
	auto killerGroup = killer->getGroup();
	if (killerGroup != nullptr) {
		...
		for (int i = 0; i < killerGroup->getGroupSize(); i++) {
			...
			if (!rikers && !inStadium && distance > 128.0f) {
				if (verboseLogging) {
	...
	float victimGroupPower = 1.0;
	int victimGroupId = 0;
	int victimGroupSize = 1;

	victimGroupPower = getPowerRating(victim);
    ...
    Vector3 killerPos = killer->getWorldPosition();

	if (verboseLogging) {
		error("threatMap size = " + String::valueOf(killerThreatMap.size()));
	}
	...
	for (int i = 0; i < killerThreatMap.size(); ++i) { 
		...
		if (attacker->isGrouped()) {
			if (attacker->getGroupID() == victimGroupId) {
				if (verboseLogging) {
					error("FRS: calculateAttackers: Participant is in killer or defender's group, not counting as part of threatmap");
				}
				continue; //don't double count group members
		...
		if (attacker->getZone() != killer->getZone()) {
   		...
   		Vector3 attackerPos = attacker->getWorldPosition();
		float distance = attackerPos.distanceTo(killerPos);
	...
	//now check for group members specifically
	auto victimGroup = victim->getGroup();
	if (victimGroup != nullptr) {
		...
		for (int i = 0; i < victimGroup->getGroupSize(); i++) {
			...
			Vector3 memberPos = member->getWorldPosition();
			float distance = memberPos.distanceTo(killerPos);
	...
	if (bhVictim) {
  		award += 500; //500 bonus for bounty hunter
		if (verboseLogging) {
  			error("Award: +500 Bounty Hunter Bonus = " + String::valueOf(award));
		}
  	}
	else if (victim->hasSkill("mandalorian_master")) {
	...
	if (!bhVictim || killerGroupPower > getPowerRating(killer)) {   // Don't modify award for solo BH kills by FRS Jedi
	...
	if (awardAllowed && !bhVictim && isSameFaction(victim, killer)) {
		if (verboseLogging) {
	...
	if (awardAllowed && victimGhost->hasBhTef() && !bhVictim) {
	...
	if (killerGroup != nullptr) {
		...
		for (int i = 0; i < killerGroup->getGroupSize(); i++) {
			CreatureObject* member = killerGroup->getGroupMember(i);
			...
			if (victimGhost->hasBhTef() && !bhVictim) {
			...
			if (!allowSameAccountPvpRatingCredit && victimAccounts.contains(memberAccountID)) {
				...
				if (verboseLogging)
					error("FRS: No member award allowed, player is character fighting for hte oppposition.  Skipping " + member->getFirstName() + ".");
				continue;
			}
	...
	if ((victim->hasSkill("force_rank_light_novice") || victim->hasSkill("force_rank_dark_novice")) && deductionXp < 0) {
		Locker clocker(frsManager, victim);
		error("FRS: Deducting " + String::valueOf(-deductionXp) + " FRS XP from victim " + victim->getFirstName());
		frsManager->adjustFrsExperience(victim, deductionXp);
	}
}
