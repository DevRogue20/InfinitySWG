...
int CreatureManagerImplementation::notifyDestruction(TangibleObject* destructor, AiAgent* destructedObject, int condition, bool isCombatAction) {
	...
	try {
		...
		for (int i = 0; i < copyThreatMap.size(); ++i) {
			...
			auto strongAttacker = attackerCreo;
			auto strongDestructed = destructedObject;
				...
				if (factionManager != nullptr && !destructedObject->getFactionString().isEmpty() && !destructedObject->isEventMob()) {
					...
					if (player->isGrouped()) {
						...
						if (group != nullptr) {
							...
							if (groupSize > 9)
								groupSize = 9;
							dividedReward /= (float)(1.0f + (groupSize/10.0f));
...
void CreatureManagerImplementation::tame(Creature* creature, CreatureObject* player, bool force, bool adult) {
	...
	uint32 pvpMask = creature->getPvpStatusBitmask();

	uint32 creatureMask = 0;
	ManagedReference<AiAgent*> agent = cast<AiAgent*>(creature);
	if (agent != nullptr) {
		creatureMask = agent->getCreatureBitmask();
	}
