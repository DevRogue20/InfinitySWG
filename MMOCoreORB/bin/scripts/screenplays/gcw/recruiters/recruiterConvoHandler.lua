					...
					if (controlledObject ~= nil) then
						...
						for i, v in pairs(imperialRewardData['hirelings']) do
							if iName == v.controlledObjectTemplate then
								createEvent(ni * 1000, "RecruiterConvoHandler", "tradeImperialStuffDestroy", pPlayer, SceneObject(pItem):getObjectID())
						...
						if not foundit then
							for i, v in pairs(rebelRewardData['hirelings']) do
								if iName == v.controlledObjectTemplate then
									createEvent((nr + ni) * 1000, "RecruiterConvoHandler", "tradeRebelStuffDestroy", pPlayer, SceneObject(pItem):getObjectID())
...
function RecruiterConvoHandler:tradeImperialStuff(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		...
		--recruiterScreenplay:awardData(pPlayer, "imperial", pItem, true)
	end
end
function RecruiterConvoHandler:tradeRebelStuff(pPlayer, pItem)
	if pPlayer ~= nil and pItem ~= nil then
		...
		--recruiterScreenplay:awardData(pPlayer, "rebel", pItem, true)
	end
end
